// /services/ari-listener.js
import AriClient from "ari-client";
import { sql, poolPromise } from "../lib/db.js";
import redis from "../lib/redis.js";
import { log } from "../lib/logger.js";
import dotenv from "dotenv";
import { checkRule } from "./business-rules.js"; // 🧩 Nuevo módulo de reglas
dotenv.config();
    ...
    async (err, ari) => {
        ...
        ari.on("StasisStart", async (event, channel, args) => {
            ...
            try {
                if (mode === "internal") {
                    ...
                    try {
                        // 1️⃣ Verificar horario
                        const inSchedule = await checkRule("schedule");
                        if (!inSchedule) {
                            log("info", `🕒 Llamada fuera de horario (${ani}) → IVR AfterHours`);
                        ...
                        // 2️⃣ Verificar feriado
                        const isHoliday = !(await checkRule("holiday"));
                        if (isHoliday) {
                            log("info", `🎉 Día feriado → IVR AfterHours`);
                            await publish(channel, "rule.applied", {
                                type: "holiday",
                                ani,
                                dnis,
                        ...
                        // 3️⃣ Verificar cliente VIP
                        const isVip = await checkRule("vip", ani);
                        if (isVip) {
                            log("info", `⭐ Cliente VIP detectado (${ani})`);
                            await publish(channel, "rule.applied", {
                                type: "vip",
                                ani,
                                dnis,
                    ...
                    // Si pasó reglas → flujo normal interno
        ...
        ari.on("ChannelDestroyed", async (event, channel) => {
            try {
                ...
                // Corte manual de A-leg huérfano
                try {
